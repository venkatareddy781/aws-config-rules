...
Scenarios:
  ...
  Scenario: 6
    Given: S3 bucket is object lock enabled with mode, days and yeaas
          (actual bucket days(days + (years*365)) should be greater than the parameter days
     Then: Return COMPLIANT
  Scenario: 7
  ...
  Scenario: 9
    Given: S3 bucket is object lock enabled with mode and years
          (actual bucket years are less than the parameter years)
     Then: Return NON_COMPLIANT
  Scenario: 10
    Given: S3 bucket is object lock enabled with mode, days and yeaas
...
class S3_OBJECT_LOCK_ENABLED(ConfigRule):
            ...
            if isexception or 'ObjectLockConfiguration' not in response:
                ...
                if mode == valid_rule_parameters.get('Mode') and totaldays >= days_in_param:
                    ...
                    evaluations.append(
                        Evaluation(ComplianceType.NON_COMPLIANT,
                                   s3_bucket_name, RESOURCE_TYPE,
                                   "ObjectLockConfiguration doesn't match."))
    ...
    def evaluate_parameters(self, rule_parameters=None):
